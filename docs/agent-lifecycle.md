...
Enforcement, all with the same acceptance (divergent copy = violation;
byte-identical copy = migration state, remove in favor of the mount):
`office/manage_tokens.py derive-agents` verifies SHA-256 identity of any
surviving copy at sync time, `crew/validate_crew_send.py` verifies the rule
plus the canonical mount on every agent service deterministically, and
`scripts/smoke.py --level 0` checks it as a static gate.
...
1. **POST** the signed message to the door (container URL when `--container`
   is used; otherwise the host URL when present, falling back to the
   container URL — instance registries generated by `derive-agents` carry
   only `container_url`).
...
The wake path itself does **not** refresh `last_active`; a woken agent's
processing of the delivered message refreshes activity, and an agent that
stays idle after wake remains subject to the idle reaper (waking is not a
liveness exemption).
