...
- [ ] `crew/crew-send.py` — wake-on-failure path: on door-down, derive the
      ...
      wake.
- [ ] `office/lifecycle/factory_control.py` — durable re-scan: XREAD
      ...
      log-only).
- [ ] `instances/*/docker-compose.yml` — mount the canonical client read-only
...
- [ ] `instances/*/OFFICE-ATTACH.md` — replace the `crew-send.py --wake` row
      (the flag does not exist) with the automatic wake-on-failure behavior.
- [ ] `instances/dev-1/crew/` and `instances/spec-1/crew/` — ensure
...
## 3. Validation
- [ ] Wake path: stop an agent → `crew-send.py developer "x"` in dev-1 →
      envelope target `dev-1-developer`, agent starts, message delivered,
      exit 0
- [ ] Wake target derivation: registry key `developer` → envelope target
...
- [ ] Durable re-scan: publish `agent.wake` while factory-control is stopped →
      controller restart processes it (idempotent), target started
- [ ] Re-delivery failure: wake succeeds but the re-delivery POST fails →
      sender exits non-zero, no silent drop
